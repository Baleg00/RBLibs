...
namespace rb::crypto
{
	...
	void sha2_384::pad_data() noexcept
	{
	...
	void sha2_384::process_data() noexcept
	{
	...
	sha2_384::sha2_384() noexcept
	{
		clear();
	}
	
	sha2_384& sha2_384::write(const void* data, size_t size) noexcept
	{
		std::copy_n(reinterpret_cast<const uint8_t*>(data), size, std::back_inserter(m_data));
		return *this;
	}
	
	void sha2_384::digest(void* dest) noexcept
	{
		pad_data();
		process_data();
		...
		for (size_t i = 0; i < 6; i++)
			bit::write_be(dest_u64++, m_hash[i]);
	}

	std::string sha2_384::hex_digest() noexcept
	{
		pad_data();
		process_data();
		...
		for (size_t i = 0; i < 6; i++)
			hex_digest << std::setw(16) << std::setfill('0') << std::hex << m_hash[i];

		return hex_digest.str();
	}
	
	void sha2_384::clear() noexcept
	{
		m_data.clear();

		m_hash[0] = 0xCBBB9D5DC1059ED8ull;
		m_hash[1] = 0x629A292A367CD507ull;
		m_hash[2] = 0x9159015A3070DD17ull;
		m_hash[3] = 0x152FECD8F70E5939ull;
		m_hash[4] = 0x67332667FFC00B31ull;
		m_hash[5] = 0x8EB44A8768581511ull;
		m_hash[6] = 0xDB0C2E0D64F98FA7ull;
		m_hash[7] = 0x47B5481DBEFA4FA4ull;
	}
	
	sha2_384& sha2_384::operator<<(char value) noexcept
	{
		m_data.push_back(value);
		return *this;
	}

	sha2_384& sha2_384::operator<<(unsigned char value) noexcept
	{
		m_data.push_back(value);
		return *this;
	}

	sha2_384& sha2_384::operator<<(short value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(unsigned short value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(int value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(unsigned int value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(long value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(unsigned long value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(long long value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(unsigned long long value) noexcept
	{
	...
	sha2_384& sha2_384::operator<<(const std::string& str) noexcept
	{
		write(str.data(), str.size());
		return *this;
	}

	std::string sha2_384::operator()(const void* data, size_t size) noexcept
	{
		clear();
		write(data, size);
		return hex_digest();
	...
	std::string sha2_384::operator()(const std::string& str) noexcept
	{
		clear();
		write(str.data(), str.size());
		return hex_digest();
